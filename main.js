document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!gl) {
        console.error('Tidak Support WebGL');
        return;
    }
    ...
    canvas.width = 800; // Ini adalah lebar dalam pixel
    canvas.height = 600; // Ini adalah tinggi dalam pixel

    // Membuat data koordinat untuk 10 titik
    const points = [
        0.0, 0.0,  // Titik 1
        0.1, 0.1,  // Titik 2
        0.2, 0.2,  // Titik 3
    ...
    const fragmentShaderSource = `
        void main() {
            gl_FragColor = vec4(0, 0, 0, 1); // Warna titik (hitam)
        }
    `;
